...
class JointCmds:
    ...
    def update( self, dt ) :
        ...
        for i, jnt in enumerate(self.joints_list):
            if(i%2) == 0:
                #self.jnt_cmd_dict[jnt] = 0.3*np.sin((i*((2*np.pi*0.6)/12)) - (2*np.pi*self.t) - np.pi/2 ) #even
                self.jnt_cmd_dict[jnt]=  0.3*np.sin((i*(0.6*2*np.pi)/12)  + (2*np.pi*self.t)+(np.pi/2))
            else:
                self.jnt_cmd_dict[jnt] = np.sin((i*(2*np.pi)/12)  + (2*np.pi*self.t))   #odd
