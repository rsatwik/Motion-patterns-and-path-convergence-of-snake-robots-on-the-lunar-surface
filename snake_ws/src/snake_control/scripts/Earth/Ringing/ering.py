    ...
    def update( self, dt ) :
        ...
        if (self.t<1):
            for i, jnt in enumerate(self.joints_list):
                if(i<7) == 0:
                    self.jnt_cmd_dict[jnt] = -1*(1.75 - mt.exp(-(self.t)/2))*np.sin(((i*np.pi)/2)) #even
                else:
                    self.jnt_cmd_dict[jnt] = -1*(1.75 - mt.exp(-(self.t)/2))*np.sin(((i*np.pi)/2))  #odd  
        elif (self.t<=1 and self.t<4):
            for i, jnt in enumerate(self.joints_list):
                if(i<7) == 0:
                    self.jnt_cmd_dict[jnt] = -1*(1.5 - mt.exp(-(self.t)/2))*np.sin(((i*np.pi)/2)) #even
                else:
                    self.jnt_cmd_dict[jnt] = -1*(1.5 - mt.exp(-(self.t)/2))*np.sin(((i*np.pi)/2))  #odd 


        else:
            for i, jnt in enumerate(self.joints_list):
                if(i%2) == 0:
                    self.jnt_cmd_dict[jnt] = -0.85*np.sin(((i*np.pi)/2) - (1.45*np.pi*self.t)) #even
                else:
                    self.jnt_cmd_dict[jnt] = -0.85*np.sin(((i*np.pi)/2) - (1.45*np.pi*self.t))  #odd
