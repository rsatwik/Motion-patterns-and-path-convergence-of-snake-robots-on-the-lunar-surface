...
class JointCmds:
    ...
    def update( self, dt ) :
        ...
        for i, jnt in enumerate(self.joints_list):
            if(i==0 or i==1) :
                self.jnt_cmd_dict[jnt] = 1.25*np.sin(((i*2*np.pi)/4) - (1.9*np.pi*self.t) + 0*np.pi/2 ) #even
            elif(i==14 or i==15):
                self.jnt_cmd_dict[jnt] = 1.25*np.sin(((i*2*np.pi)/4)  + (1.9*np.pi*self.t))  #odd
            else:
                self.jnt_cmd_dict[jnt] = 0*np.sin(((i*2*np.pi)/4)  + (1.9*np.pi*self.t))      
