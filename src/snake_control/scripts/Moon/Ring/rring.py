    ...
    def update( self, dt ) :
        ...
        if (self.t<2):
            for i, jnt in enumerate(self.joints_list):
                if(i<7) == 0:
                    self.jnt_cmd_dict[jnt] = 0.5*np.sin(((i*np.pi)/2) + (0*np.pi*self.t) + 0*np.pi/2 ) #even
                else:
                    self.jnt_cmd_dict[jnt] = 0.5*np.sin(((i*np.pi)/2)  + (0*np.pi*self.t))  #odd  


        else:
            for i, jnt in enumerate(self.joints_list):
                if(i<7) == 0:
                    self.jnt_cmd_dict[jnt] = 1.2*np.sin(((i*np.pi)/2) + (1.9*np.pi*self.t) + 0*np.pi/2 ) #even
                else:
                    self.jnt_cmd_dict[jnt] = 1.2*np.sin(((i*np.pi)/2)  + (1.9*np.pi*self.t))  #odd  
