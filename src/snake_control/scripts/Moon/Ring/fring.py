    ...
    def update( self, dt ) :
        ...
        if (self.t<4):
            for i, jnt in enumerate(self.joints_list):
                if(i<7) == 0:
                    self.jnt_cmd_dict[jnt] = -0.8*(1 - mt.exp(-(self.t)/1.25))*np.sin(((i*np.pi)/2)) #even
                else:
                    self.jnt_cmd_dict[jnt] = -0.8*(1 - mt.exp(-(self.t)/1.25))*np.sin(((i*np.pi)/2))  #odd  


        else:
            for i, jnt in enumerate(self.joints_list):
                if(i%2) == 0:
                    self.jnt_cmd_dict[jnt] = -1*np.sin(((i*np.pi)/2) - (1.9*np.pi*self.t)) #even
                else:
                    self.jnt_cmd_dict[jnt] = -1*np.sin(((i*np.pi)/2) - (1.9*np.pi*self.t))  #odd  
