    ...
    def update( self, dt ) :
        ...
        for i, jnt in enumerate(self.joints_list):
            if(i==0 or i==1 or i==2 ) :
                self.jnt_cmd_dict[jnt] = 0.5*np.sin(((i*np.pi)/2) + (1.9*np.pi*self.t) + 0*np.pi/2 ) #even
            elif(i==14 or i==15 or i==13):
                self.jnt_cmd_dict[jnt] = 0.5*np.sin(((i*np.pi)/2)  - (1.9*np.pi*self.t))  #odd
            else:
                self.jnt_cmd_dict[jnt] = 0.6*np.sin(((i*np.pi)/2)  + (0*np.pi*self.t))      
